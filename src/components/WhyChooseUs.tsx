...
export default function WhyChooseUs() {
  ...
  return (
    <section id="por-que-elegirnos" className="bg-gray-50 py-16 sm:py-20 lg:py-28">
      <div className="container-px">
        <div className="mt-10 grid grid-cols-1 gap-5 sm:grid-cols-2 sm:gap-6 lg:grid-cols-4 lg:mt-14">
          {items.map((item, i) => {
            const Icon = item.icon;
            return (
              <div
                key={item.titleKey}
                className={`reveal reveal-delay-${(i % 4) + 1} group relative overflow-hidden rounded-2xl border border-gray-100 bg-white p-5 transition-all duration-500 hover:-translate-y-2 hover:border-primary/30 hover:shadow-2xl hover:shadow-primary/10 sm:p-7`}
              >
